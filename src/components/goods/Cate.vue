...
<div>
    ...
    <el-card>
        ...
        <!-- 表格 :data数据源 :columns表格各列的配置 -->
        <!-- show-index显示索引列 index-text索引列标题 selection-type多选 expand-type展开 -->
    ...
    <el-dialog title="提示" :visible.sync="addCateDialogVisible" width="50%" @close="addCateDialogClosed">
        ...
        <el-form :model="addCateForm" :rules="addCateFormRules" ref="addCateFormRef" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item label="父级分类：">
                <!-- options 用来指定数据源 -->
                <!-- props 用来指定配置对象 -->
                <!-- v-model 将我们选中的值双向绑定到data中 必须是个数组 -->
                <!-- clearable	是否支持清空选项 -->
                <el-cascader v-model="selectedKeys" :options="parentCateList" :props="cascaderProps" @change="parentCateChanged" clearable size="mini"></el-cascader>
            </el-form-item>
        </el-form>
...
export default {
    ...
    methods: {
        ...
        async getParentCateList() {
            // 获取到前2级分类就行 因为一共就3级添加到第3级只需要选择到2级分类就行
            const {
                data: res
        ...
        parentCateChanged() {
            ...
            if(this.selectedKeys.length > 0){
                // 父级分类的Id 永远取选中项的最后一个值
                this.addCateForm.cat_pid=this.selectedKeys[this.selectedKeys.length-1];
                // 为当前分类的等级赋值
                this.addCateForm.cat_level=this.selectedKeys.length;
                return
            }else{
                // 反之，就说明没有选中任何父级分类 没选就重置初始为1级状态
